...
const communityengagement = () => {
  ...
  const category = "community_engagement"; // Category for fetching images
  ...
  useEffect(() => {
    fetch(`${SERVER_URL}/api/photos/${category}`)
      .then((res) => res.json())
      .then((data) => {
        console.log("Fetched communityengagement images:", data);
      ...
      .catch((err) => {
        console.error("Error fetching communityengagement images:", err);
      });
  }, []);
  ...
  const handleUpload = async () => {
    try {
      ...
      if (lastLoginTime && now - parseInt(lastLoginTime) < FIVE_MINUTES) {
        navigation.navigate("community_engagement");
      } else {
        navigation.navigate("communityengagement_login");
      }
    } catch (error) {
