...
const socialmedia = () => {
  ...
  const category = "socialmedia"; // Category for fetching images
  ...
  useEffect(() => {
    fetch(`${SERVER_URL}/api/photos/${category}`)
      .then((res) => res.json())
      .then((data) => {
        console.log("Fetched socialmedia images:", data);
      ...
      .catch((err) => {
        console.error("Error fetching socialmedia images:", err);
      });
  }, []);
  ...
  const handleUpload = async () => {
    try {
      ...
      if (lastLoginTime && now - parseInt(lastLoginTime) < FIVE_MINUTES) {
        ...
        navigation.navigate("socialmedia_login");
      }
    } catch (error) {
