...
const socialactivities = () => {
  ...
  const category = "social_activities";
  ...
  useEffect(() => {
    fetch(`${SERVER_URL}/api/photos/${category}`)
      .then((res) => res.json())
      .then((data) => {
        console.log("Fetched socialactivities images:", data);
      ...
      .catch((err) => {
        console.error("Error fetching socialactivities images:", err);
      });
  }, []);
  ...
  const handleUpload = async () => {
    try {
      ...
      if (lastLoginTime && now - parseInt(lastLoginTime) < FIVE_MINUTES) {
        navigation.navigate("social_activities");
      } else {
        navigation.navigate("socialactivities_login");
      }
    } catch (error) {
